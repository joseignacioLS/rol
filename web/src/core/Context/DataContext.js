...
const reducer = (state, action) => {
  ...
  switch (type) {
    case "set":
      newState.data[ payload.title ] = payload.body;
      newState.selected = payload.title;
      return newState;
    case "select":
...
const DataProvider = ({ children }) => {
  ...
  return <DataContext.Provider
    value={{ gameData: data[ selected ], selected, data: selectedData, path: path, listOfPages: getListOfPages(data[ selected ]), dataDispatcher }}
  >
    {children}
